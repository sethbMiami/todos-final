...
app.post("/users/signin", async (req, res) => {
  ...
  let authorized = await res.locals.store.signIn(username, password);
...
app.post("/lists",
  ...
  catchError(async (req, res) => {
    ...
    if (!errors.isEmpty()) {
      ...
      let created = await res.locals.store.createList(req.body.todoListTitle);
...
app.get("/lists/:todoListId",
  ...
  catchError(async (req, res) => {
    ...
    await res.locals.store.sortedTodos(todoList);
...
app.post("/lists/:todoListId/complete_all",
  ...
  catchError(async (req, res) => {
    let todoListId = req.params.todoListId;
    let markAllDone = await res.locals.store.markAllDone(+todoListId);
    //let todoList = res.locals.store.loadTodoList(+todoListId);
    if (!markAllDone) throw new Error("Not found.");
